...
if __name__ == '__main__':
    ...
    BRAM_depth = 2**12 # 4096 Addresses
    BRAM_data_width = 4 # Bytes
    BRAM_offset = 0 # Read from addr 0
    amp = fpga.read('amp_EQ0_coeff_bram',BRAM_depth*BRAM_data_width,0)   
    amp += fpga.read('amp_EQ1_coeff_bram',BRAM_depth*BRAM_data_width,0)   
    amp += fpga.read('amp_EQ2_coeff_bram',BRAM_depth*BRAM_data_width,0)   
    amp += fpga.read('amp_EQ3_coeff_bram',BRAM_depth*BRAM_data_width,0) 

    phase = fpga.read('phase_EQ0_coeff_bram',BRAM_depth*BRAM_data_width,0)
    phase += fpga.read('phase_EQ1_coeff_bram',BRAM_depth*BRAM_data_width,0)
    phase += fpga.read('phase_EQ2_coeff_bram',BRAM_depth*BRAM_data_width,0)
    phase += fpga.read('phase_EQ3_coeff_bram',BRAM_depth*BRAM_data_width,0)

    ofile = open('amp_phase_coeff_from_jack.dat','wb')
    ofile.write(amp)
    ofile.write(phase)
    ofile.close()
