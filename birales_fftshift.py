...
if __name__ == '__main__':
    ...
    p = OptionParser()
    p.set_usage('birales_fftshift.py <ROACH_HOSTNAME_or_IP> [options]')
    p.set_description(__doc__)
    p.add_option('-c', '--config', dest='config_file',default="configura_birales.conf",
        help='Select the Configuration file')
    p.add_option('-f', '--fftshift', dest='fft_shift', type='int',default='0',
        help='FFT Shift (Default: 0)')
    ...
    ans = read_header('fft_shift',header)
    print "\nRead from header fft_shift=%d  (%s)"%(ans,bin(ans))
    if not opts.fft_shift == 0:
        time.sleep(0.1)
        set_fft_shift(opts.fft_shift)
    
        print "Updating header BRAM with %s=%d"%('fft_shift',opts.fft_shift)
        write_header('fft_shift',opts.fft_shift,header)
        ans = read_header('fft_shift',header)
        print "Read from header fft_shift=%d  (%s)"%(ans,bin(ans))
