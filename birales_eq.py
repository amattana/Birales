    ...
    for pn,pol in enumerate(['x','y'][0:1]):
        for eq_subsys in range(32//8):
            ...
            for ant_mux_index in range(8):
                offset = (1024/4)/2*MAP*4 #Offset in bytes, not words
                if verbose:
...
if __name__ == '__main__':
    ...
    amp_coeffs = n.zeros([32,1024/4],dtype=float) #32 antennas, 1024 chans
    valori = []
    indice = []
    for coeff in coeffs:
        c = coeff[:-1].split()
    ...
    print 'Loading Phase calibration file: %s --> ' %opts.phs_cal,
    phsfile = open(opts.phs_cal,'r')
    coeffs = phsfile.readlines()
    phs_coeffs = n.zeros([32,1024/4],dtype=complex) #32 ants, 1024 chans
    valori = []
    indice = []
    for coeff in coeffs:
        c = coeff[:-1].split()
...
#    for ant in range(fConf.n_ants_sp):
    new_phs_coeffs = n.ones((32,1,512/4),dtype=complex)
    new_amp_coeffs = n.ones((32,1,512/4),dtype=float)
    print 'Modifying calibration coefficients... ',
    for ant in range(32):
        #print  ',[%d]'%(ant),
