    ...
    def new_capture(self,sw_trig=False):
        for fpga in self.fpgas:
            ...
            while (fpga.read_int(self.addr)!=(self.depth-1)):
                ...
                time.sleep(0.01)
        #return [fpga.read(self.bram,self.depth*4) for fpga in self.fpgas] changed for birales
        return [fpga.read(self.bram,self.depth) for fpga in self.fpgas]
...
class Spectras18(SnapBlock):
    def __init__(self,fpgas,ram_path='',ctrl_path='',addr_path='',depth=1024,input_sel='',sync_sel='',sync_delay='',n_chans=256,n_ants=32,n_pols=1,sync_index=0, quiet=False):
        ...
        self.n_snaps = 32#self.n_pols*self.n_ants*self.n_chans/self.depth
        self.chans_per_snap = 1#self.depth/self.n_chans
    ...
    def get_spectras(self):
        ...
        for sel in range(self.n_snaps):
            ...
            #print sel, self.n_snaps, self.sync_index, self.chans_per_snap, self.depth,self.n_chans,sel//(8/self.chans_per_snap),len(data) 
    ...
    def unpack(self,data):
        ...
        data_list = [np.array(s.unpack('>%dh'%(2*self.depth/4),data[fn])) for fn in range(self.n_fpgas)]
...
class SpectrasQuant(SnapBlock):
    def __init__(self,fpgas,ram_path='',ctrl_path='',addr_path='',depth=1024,input_sel='',sync_sel='',sync_delay='',n_chans=256,n_ants=32,n_pols=1,sync_index=0,ants_per_word=4,quiet=False, data_index=0):
        ...
        self.sync_index = sync_index
        self.n_snaps = 32#self.n_pols*self.n_ants*self.n_chans/(self.depth*self.ants_per_word)
        self.chans_per_snap = 1#self.depth/self.n_chans
    ...
    def unpack(self,data):
        ...
        mask = (2**word_width)-1
        data_list = [np.array(s.unpack('>%dL'%(self.depth/4),data[fn])) for fn in range(self.n_fpgas)]
...
class SpectrasTranspose(SnapBlock):
    ...
    def unpack(self,data):
        word_width = int(32/self.ants_per_word)
        mask = (2**word_width)-1
        data_list = [np.array(s.unpack('>%dB'%(self.depth/4*self.ants_per_word),data[fn])) for fn in range(self.n_fpgas)]
        ret_data = np.array([])
        for fn in range(self.n_fpgas):
